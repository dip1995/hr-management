...
export class DailyWorkListComponent implements OnInit {
    ...
    this.employeeService
      ...
      .subscribe((res) => {
        if (res.status) {
          ...
          this.reportData.total_leaves = this.reportData.total_leaves
            ? this.reportData.total_leaves
            : 0;
          this.reportData.total_working_days = this.reportData
            .total_working_days
            ? this.reportData.total_working_days
            : 0;
          this.reportData.total_days = this.reportData.total_days
            ? this.reportData.total_days
            : 0;
          this.reportData.absent = this.reportData.absent
            ? this.reportData.absent
            : 0;
          this.reportData.present = this.reportData.present
            ? this.reportData.present
            : 0;
          if (
            this.grouped_dailyWork &&
            Object.keys(this.grouped_dailyWork).length > 0
          ) {
  ...
  makePresentProcess() {
    if (this.currentRow) {
      let obj = {
        date: this.currentRow.ea_date
          ? this.currentRow.ea_date
          : this.currentRow.date,
