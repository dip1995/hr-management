...
export class HrHeaderComponent implements OnInit {
  ...
  constructor(
    ...
    this.userCookie = this.cookieService.get('superadmin')
      ? JSON.parse(this.cookieService.get('superadmin'))
      : {};
  }
  ...
  getAllEmployeeReportCard() {
    this.superadminService.getAllEmployeeReportCard({}).subscribe((res) => {
      if (res.data && res.data.length > 0) {
        ...
        this.employeeReport.total_leave = this.employeeReport.total_leave
          ? this.employeeReport.total_leave
          : 0;
        this.employeeReport.pending = this.employeeReport.pending
          ? this.employeeReport.pending
          : 0;
        this.employeeReport.rejected = this.employeeReport.rejected
          ? this.employeeReport.rejected
          : 0;
        this.employeeReport.approved = this.employeeReport.approved
          ? this.employeeReport.approved
          : 0;
      }
    });
  }
