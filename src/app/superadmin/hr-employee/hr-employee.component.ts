...
export class HrEmployeeComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      },
      {
        ...
        cellRenderer: (params) => {
          return params.value
            ? '<a class="ag-grid-link"><i class="fa fa-download"></i> Download</a>'
            : '';
        },
      },
      {
      ...
      },
      {
        ...
        cellRenderer: (params) => {
          return '<a class="ag-grid-link"><i class="fa fa-pencil"></i>';
        },
      },
      // {
      //   headerName: 'Action',
      ...
      //   },
      //   cellRenderer: (params) => {
      //     return '<a class="ag-grid-link"><i class="fa fa-trash-o"></i>';
      //   },
      // },
    ];
  ...
  ngOnInit(): void {
    this.userCookie = this.cookieService.get('epsuperadmin')
      ? JSON.parse(this.cookieService.get('epsuperadmin'))
      : {};
    this.hrEmployeeList();
  }
  ...
  addUpdateEmployeeDetails(addUpdateEmployeeForm) {
    ...
    if (addUpdateEmployeeForm.status == 'VALID') {
      ...
      this.superadminService
        .addUpdateEmployeeDetails(this.employee_data)
        .subscribe((res) => {
          if (res.status) {
            this.isSubmit = false;
  ...
  //       id: selected[0].userid,
  //     };
  // let row_ids = selected.map(function (d: any) {
  //   return d.row_id;
  // });
  // let obj = {
  //   row_ids: row_ids,
  // };
  //     this.superadminService.deleteEmployeeById(obj).subscribe((res) => {
  //       if (res.status) {
  //         this.alertSuccessErrorMsg(res.status, res.message, false);
  //       } else {
  //         this.alertSuccessErrorMsg(res.status, res.message, false);
      ...
      if (res.status) {
        if (res.data && res.data.length > 0) {
          ...
          this.employee_data = {
            ...
            start_date: this.convertDateToReadableString(
              res.data[0].start_date
            ),
            increament_date: this.convertDateToReadableString(
              res.data[0].increament_date
            ),
